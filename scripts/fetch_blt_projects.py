...
def is_relevant_project(item):
    ...
    project_type = (
        (item.get("type", "") or "").lower() if isinstance(item.get("type"), str) else ""
    )
...
def main():
    ...
    blt_count = len(
        [k for k in all_projects.keys() if k in [r["name"].lower() for r in blt_repos]]
    )
    ...
    blt_samples = [
        (k, v)
        for k, v in list(all_projects.items())[:15]
        if "blt" in k.lower() or "blt" in v[0].lower()
    ]
